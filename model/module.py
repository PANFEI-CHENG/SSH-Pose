...
from layer import conv3x3, SPA_SMaxPool, incept_SConv
...
class InceptionBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, dilation=1, type='spa_sconv'):
        ...
        if stride > 1:
            ...
            self.conv1 = incept_SConv(inplanes, planes, kernel_size=3, band_kernel_size=9, branch_ratio=0.125)
        self.bn1 = nn.BatchNorm2d(planes)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = incept_SConv(planes, planes, kernel_size=3, band_kernel_size=9, branch_ratio=0.125)
        self.bn2 = nn.BatchNorm2d(planes)
        self.downsample = downsample
        self.stride = stride
...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x2 = self.layer1(x2)#32,64-32,64*
        x3 = self.layer2(x2)#32,64-16,128*
        x4 = self.layer3(x3)#16,128-8,256
        x5 = self.layer4(x4)#8,256-8,256*

        return x2, x3, x5  #32, 16, 8
...
class SphericalFPN3(nn.Module):
    def __init__(self, dim_in1=1, dim_in2 = 3, dim_in3 = 384,type='spa_sconv', ds_rate=2):
        ...
        self.ds_rate = ds_rate
        assert ds_rate in [1,2,4]
        self.encoder1 = ResNet(InceptionBlock, dim_in1, [2, 2, 2, 2], type=type)
        self.encoder2 = ResNet(InceptionBlock, dim_in2, [2, 2, 2, 2], type=type)
        self.encoder3 = ResNet(InceptionBlock, dim_in3, [2, 2, 2, 2], type=type)
        ...
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                ...
                # 对 nn.LayerNorm 进行初始化
                m.weight.data.fill_(1)
                m.bias.data.zero_()

    def forward(self, x1, x2, x3):
        y11,y21,y31 = self.encoder1(x1)#32,64;16,128;8,256
        y12,y22,y32 = self.encoder2(x2)

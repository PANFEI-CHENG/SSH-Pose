...
def _write_evaluation_report(report, logger=None, report_path=None, mode='w'):
    """Write the same evaluation report to the console/logger and a text file."""
    ...
    if report_path is not None:
        ...
        with open(report_path, mode, encoding='utf-8') as report_file:
            if mode == 'a' and report_file.tell() > 0:
                report_file.write('\n')
            report_file.write(report.rstrip() + '\n')


def _match_count(pred_matches, gt_matches):
    """Return the integer counts associated with one evaluation threshold."""
    matched = int(np.count_nonzero(pred_matches > -1))
    predictions = int(pred_matches.size)
    ground_truth = int(gt_matches.size)
    return {
        'matched': matched,
...
def _pooled_match_count(class_counts):
    """Pool integer counts over categories (this is not macro AP)."""
    matched = sum(item['matched'] for item in class_counts)
    predictions = sum(item['predictions'] for item in class_counts)
    ground_truth = sum(item['ground_truth'] for item in class_counts)
    return {
        'matched': matched,
        'predictions': predictions,
        'ground_truth': ground_truth,
...
def _compute_ap_breakdown(pred_matches, pred_scores, gt_matches):
    """Expose the exact terms used by compute_ap_from_matches_scores().

    With VOC-style interpolated precision, AP can be written as the sum of
    interpolated precision at every true-positive recall jump, divided by the
    number of GT instances. The returned numerator is therefore generally a
    floating-point value, not simply the number of matches.
    """
    count = _match_count(pred_matches, gt_matches)
    gt_total = count['ground_truth']
    if gt_total == 0:
        return {
            **count,
            'ap_precision_sum': 0.0,
            'ap_denominator_gt': 0,
            'ap_recomputed_percent': 0.0,
        }

    score_indices = np.argsort(pred_scores)[::-1]
    sorted_is_match = pred_matches[score_indices] > -1
    precisions = np.cumsum(sorted_is_match) / (
        np.arange(sorted_is_match.size) + 1)
    # Match compute_ap_from_matches_scores exactly (including float32 recall).
    recalls = np.cumsum(sorted_is_match).astype(np.float32) / gt_total

    padded_precisions = np.concatenate([[0.0], precisions, [0.0]])
    padded_recalls = np.concatenate([[0.0], recalls, [1.0]])
    for index in range(padded_precisions.size - 2, -1, -1):
        padded_precisions[index] = max(
            padded_precisions[index], padded_precisions[index + 1])
    recall_change_indices = np.where(
        padded_recalls[:-1] != padded_recalls[1:])[0] + 1
    recall_deltas = (padded_recalls[recall_change_indices] -
                     padded_recalls[recall_change_indices - 1])
    contributions = recall_deltas * padded_precisions[recall_change_indices]
    ap = float(np.sum(contributions))
    # Each true-positive recall jump is 1 / GT. A final unmatched-recall jump,
    # when present, has zero precision and therefore contributes zero.
    ap_precision_sum = ap * gt_total
    return {
        **count,
        'ap_precision_sum': ap_precision_sum,
        'ap_denominator_gt': gt_total,
        'ap_recomputed_percent': ap * 100.0,
    }


def _format_count_suffix(count):
    if count.get('aggregation') == 'macro':
        return ('macro AP=({category_ap_formula})/{category_count}; '
                'pooled matched={matched}/{ground_truth} GT, '
                'pred={predictions}').format(**count)
    if count['ap_denominator_gt'] == 0:
        return ('AP formula: no GT -> AP=0; matched={matched}/0 GT, '
                'pred={predictions}').format(**count)
    return ('AP formula: 100 * {ap_precision_sum:.6f} / '
            '{ap_denominator_gt}; matched={matched}/{ground_truth} GT, '
            'pred={predictions}').format(**count)


def _format_independent_map_report(iou_3d_aps, pose_aps, synset_names,
                                   iou_thres_list, degree_thres_list,
                                   shift_thres_list, raw_counts=None):
    """Create compact overall/per-category metric tables."""
    rows = [(cls_id, synset_names[cls_id])
            for cls_id in range(1, len(synset_names))]
    rows.append((len(synset_names), 'mean'))
    degree_only_shift_index = shift_thres_list.index(100)
    shift_only_degree_index = degree_thres_list.index(360)

    def table(title, headers, values):
        category_width = max(10, max(len(name) for _, name in rows))
        value_width = max(10, max(len(header) for header in headers))
        header = '{:<{}} | {}'.format(
            'category', category_width,
            ' | '.join('{:>{}}'.format(item, value_width)
                       for item in headers))
        output = ['\n' + title, header, '-' * len(header)]
        for row_id, class_name in rows:
            output.append('{:<{}} | {}'.format(
                class_name, category_width,
                ' | '.join('{:>{}.1f}'.format(value, value_width)
                           for value in values(row_id))))
        return output

    finite_degrees = [(index, degree)
                      for index, degree in enumerate(degree_thres_list)
                      if degree != 360]
    finite_shifts = [(index, shift)
                     for index, shift in enumerate(shift_thres_list)
                     if shift != 100]
    lines = [
        '=' * 80,
        'Independent mAP evaluation report (AP, %)',
        '=' * 80,
    ]
    lines.extend(table(
        '3D IoU',
        ['IoU@{:.0f}'.format(thres * 100) for thres in iou_thres_list],
        lambda row_id: [value * 100 for value in iou_3d_aps[row_id]]))
    lines.extend(table(
        'Rotation (translation ignored)',
        ['{}deg'.format(degree) for _, degree in finite_degrees],
        lambda row_id: [pose_aps[row_id, index,
                                 degree_only_shift_index] * 100
                        for index, _ in finite_degrees]))
    lines.extend(table(
        'Translation (rotation ignored)',
        ['{}cm'.format(shift) for _, shift in finite_shifts],
        lambda row_id: [pose_aps[row_id, shift_only_degree_index,
                                 index] * 100
                        for index, _ in finite_shifts]))
    pose_columns = [(degree_index, shift_index, degree, shift)
                    for degree_index, degree in finite_degrees
                    for shift_index, shift in finite_shifts]
    lines.extend(table(
        'Pose (rotation + translation)',
        ['{}deg/{}cm'.format(degree, shift)
         for _, _, degree, shift in pose_columns],
        lambda row_id: [pose_aps[row_id, degree_index, shift_index] * 100
                        for degree_index, shift_index, _, _ in pose_columns]))
    return '\n'.join(lines)


def _save_independent_raw_data(output_dir, synset_names, iou_thres_list,
                               degree_thres_list, shift_thres_list,
                               iou_3d_aps, pose_aps,
                               iou_pred_matches_all, iou_pred_scores_all,
                               iou_gt_matches_all, pose_pred_matches_all,
                               pose_pred_scores_all, pose_gt_matches_all):
    """Save aggregate integer counts and exact arrays used to compute AP."""
    raw_counts = {'iou': {}, 'pose': {}}
    summary_rows = []
    raw_arrays = {
        'independent_iou_thresholds': np.asarray(iou_thres_list),
        'independent_degree_thresholds': np.asarray(degree_thres_list),
        'independent_shift_thresholds': np.asarray(shift_thres_list),
    }
    num_classes = len(synset_names)

    for cls_id in range(1, num_classes):
        class_name = synset_names[cls_id]
        for index, iou_thres in enumerate(iou_thres_list):
            pred_matches = iou_pred_matches_all[cls_id][index, :]
            gt_matches = iou_gt_matches_all[cls_id][index, :]
            count = _compute_ap_breakdown(
                pred_matches, iou_pred_scores_all[cls_id][index, :],
                gt_matches)
            raw_counts['iou'][(cls_id, index)] = count
            summary_rows.append({
                'evaluator': 'independent', 'scope': 'category',
                'category': class_name, 'metric': '3d_iou',
                'iou_threshold': iou_thres,
                'rotation_threshold_deg': np.nan,
                'translation_threshold_cm': np.nan,
                **count, 'ap_percent': iou_3d_aps[cls_id, index] * 100,
                'ap_aggregation': 'confidence-ranked PR curve',
                'ap_formula': ('no GT -> 0' if count['ground_truth'] == 0 else
                               '100 * {:.6f} / {}'.format(
                                   count['ap_precision_sum'],
                                   count['ap_denominator_gt'])),
            })
            prefix = 'independent_iou_{}_{}'.format(class_name, index)
            raw_arrays[prefix + '_pred_matches'] = pred_matches
            raw_arrays[prefix + '_pred_scores'] = iou_pred_scores_all[cls_id][index, :]
            raw_arrays[prefix + '_gt_matches'] = gt_matches

        for degree_index, degree in enumerate(degree_thres_list):
            for shift_index, shift in enumerate(shift_thres_list):
                if degree == 360 and shift == 100:
                    continue
                if degree == 360:
                    metric = 'translation'
                elif shift == 100:
                    ...
                    metric = 'pose'
                pred_matches = pose_pred_matches_all[cls_id][degree_index,
                                                              shift_index, :]
                gt_matches = pose_gt_matches_all[cls_id][degree_index,
                                                        shift_index, :]
                count = _compute_ap_breakdown(
                    pred_matches,
                    pose_pred_scores_all[cls_id][degree_index,
                                                 shift_index, :],
                    gt_matches)
                raw_counts['pose'][(cls_id, degree_index, shift_index)] = count
                summary_rows.append({
                    'evaluator': 'independent', 'scope': 'category',
                    'category': class_name, 'metric': metric,
                    'iou_threshold': np.nan,
                    'rotation_threshold_deg': degree if degree != 360 else np.nan,
                    'translation_threshold_cm': shift if shift != 100 else np.nan,
                    **count,
                    'ap_percent': pose_aps[cls_id, degree_index, shift_index] * 100,
                    'ap_aggregation': 'confidence-ranked PR curve',
                    'ap_formula': ('no GT -> 0' if count['ground_truth'] == 0 else
                                   '100 * {:.6f} / {}'.format(
                                       count['ap_precision_sum'],
                ...
                prefix = 'independent_pose_{}_{}_{}'.format(
                    class_name, degree_index, shift_index)
                raw_arrays[prefix + '_pred_matches'] = pred_matches
                raw_arrays[prefix + '_pred_scores'] = pose_pred_scores_all[
                    cls_id][degree_index, shift_index, :]
                raw_arrays[prefix + '_gt_matches'] = gt_matches

    mean_row_id = num_classes
    for index, iou_thres in enumerate(iou_thres_list):
        count = _pooled_match_count([
            raw_counts['iou'][(cls_id, index)]
            for cls_id in range(1, num_classes)])
        category_ap_values = [
            iou_3d_aps[cls_id, index] * 100
            for cls_id in range(1, num_classes)]
        count.update({
            'aggregation': 'macro',
            'category_ap_formula': ' + '.join(
                '{:.6f}'.format(value) for value in category_ap_values),
            'category_count': num_classes - 1,
            'ap_precision_sum': np.nan,
            'ap_denominator_gt': np.nan,
            'ap_recomputed_percent': np.mean(category_ap_values),
        })
        raw_counts['iou'][(mean_row_id, index)] = count
        summary_rows.append({
            'evaluator': 'independent', 'scope': 'overall_macro',
            'category': 'mean', 'metric': '3d_iou',
            'iou_threshold': iou_thres,
            'rotation_threshold_deg': np.nan,
            'translation_threshold_cm': np.nan,
            **count, 'ap_percent': iou_3d_aps[mean_row_id, index] * 100,
            'ap_aggregation': 'mean of {} category AP values'.format(
                num_classes - 1),
            'ap_formula': '({}) / {}'.format(
                count['category_ap_formula'], count['category_count']),
        })
    ...
    for degree_index, degree in enumerate(degree_thres_list):
        for shift_index, shift in enumerate(shift_thres_list):
            ...
            if degree == 360:
                ...
                metric = 'pose'
            count = _pooled_match_count([
                raw_counts['pose'][(cls_id, degree_index, shift_index)]
                for cls_id in range(1, num_classes)])
            category_ap_values = [
                pose_aps[cls_id, degree_index, shift_index] * 100
                for cls_id in range(1, num_classes)]
            count.update({
                'aggregation': 'macro',
                'category_ap_formula': ' + '.join(
            ...
            raw_counts['pose'][(mean_row_id, degree_index, shift_index)] = count
            summary_rows.append({
                'evaluator': 'independent', 'scope': 'overall_macro',
                'category': 'mean', 'metric': metric,
                'iou_threshold': np.nan,
                'rotation_threshold_deg': degree if degree != 360 else np.nan,
                'translation_threshold_cm': shift if shift != 100 else np.nan,
                **count,
                'ap_percent': pose_aps[mean_row_id, degree_index,
                                       shift_index] * 100,
                'ap_aggregation': 'mean of {} category AP values'.format(
                    num_classes - 1),
                'ap_formula': '({}) / {}'.format(
    ...
    if output_dir is not None:
        ...
        pd.DataFrame(summary_rows).to_csv(
            os.path.join(output_dir, 'evaluation_raw_counts.csv'), index=False)
        np.savez_compressed(
            os.path.join(output_dir, 'evaluation_raw_matches.npz'),
            **raw_arrays)
    return raw_counts
...
def compute_3d_iou_new(RT_1, RT_2, scales_1, scales_2, handle_visibility, class_name_1, class_name_2):
    ...
    if (class_name_1 in ['bottle', 'bowl', 'can'] and class_name_1 == class_name_2) or (class_name_1 == 'mug' and class_name_1 == class_name_2 and handle_visibility == 0):
        ...
        noc_cube_1 = get_3d_bbox(scales_1, 0)#3D包围盒的八个点
        noc_cube_2 = get_3d_bbox(scales_2, 0)
        bbox_3d_2 = transform_coordinates_3d(noc_cube_2, RT_2)#真值的包围盒变换到相机坐标系下
        ...
        n = 20
        max_iou = 0
        for i in range(n):#对于对称物体绕y轴的旋转分成20份
...
def compute_3d_matches(gt_class_ids, gt_RTs, gt_scales, gt_handle_visibility, synset_names,
    ...
    for i in range(num_pred):
        for j in range(num_gt):
            ...
            #    synset_names[pred_class_ids[i]], synset_names[gt_class_ids[j]])
            overlaps[i, j] = compute_3d_iou_new(pred_RTs[i], gt_RTs[j], pred_scales[i, :], gt_scales[j],
                                                gt_handle_visibility[j], synset_names[pred_class_ids[i]], synset_names[gt_class_ids[j]])#计算三维交并比
    ...
    for s, iou_thres in enumerate(iou_3d_thresholds):
        for i in range(len(pred_boxes)):
            ...
            if low_score_idx.size > 0:
                sorted_ixs = sorted_ixs[:low_score_idx[0]]#如果有小于阈值的删掉，此处阈值为0
            # 3. Find the match
            for j in sorted_ixs:
...
def compute_independent_mAP(final_results, synset_names, degree_thresholds=[360], shift_thresholds=[100], iou_3d_thresholds=[0.1], iou_pose_thres=0.1, use_matches_for_pose=True, logger=None, report_path=None, save_raw_details=False, save_overlap_excel=True):
    ...
    for progress, result in tqdm(enumerate(final_results)):
        ...
        for cls_id in range(1, num_classes):
            ...
            # 填充 DataFrame
            for i, RT in enumerate(RT_overlaps):
                data['progress'].append(progress)  # 从1开始
                data['pkl_path'].append(os.path.basename(result['pkl_path']))
                data['cls_id'].append(cls_id)
                data['instance_id'].append(i)  # pred instance id
                data['R(du)'].append(RT[0,0])
                data['t(cm)'].append(RT[0,1])
    ...
    if save_overlap_excel:
        df.to_excel('RT_overlaps_output.xlsx', index=False)
    iou_dict = {}
        ...
        for j, shift_thres in enumerate(shift_thres_list):
            for cls_id in range(1, num_classes):
                ...
                pose_aps[cls_id, i, j] = compute_ap_from_matches_scores(cls_pose_pred_matches_all,
                                                                        ...
                                                                        cls_pose_gt_matches_all)#按例平均

            # import ipdb; ipdb.set_trace()

            pose_aps[-1, i, j] = np.mean(pose_aps[1:-1, i, j])#按类别平均
    
    if save_raw_details:
        output_dir = ((os.path.dirname(report_path) or '.')
                      if report_path is not None else None)
        _save_independent_raw_data(
            output_dir, synset_names, iou_thres_list, degree_thres_list,
            shift_thres_list, iou_3d_aps, pose_aps,
            iou_pred_matches_all, iou_pred_scores_all, iou_gt_matches_all,
            pose_pred_matches_all, pose_pred_scores_all, pose_gt_matches_all)
    report = _format_independent_map_report(
        iou_3d_aps, pose_aps, synset_names, iou_thres_list,
        degree_thres_list, shift_thres_list)
    _write_evaluation_report(report, logger=logger, report_path=report_path)
...
def evaluate(path, logger=None):
    ...
    report_path = os.path.join(path, 'evaluation_report.txt')
    ...
    report_message = 'Evaluation report saved to: {}'.format(report_path)
    if logger is not None:
        logger.warning(report_message)
    else:

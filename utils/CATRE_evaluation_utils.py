...
def compute_CATRE_IoU(
    ...
    import pandas as pd
    file_path = 'RT_overlaps_output.xlsx'
    ...
    # Save both the aggregate integer counts and the exact arrays used by AP.
    from evaluation_utils import (_compute_ap_breakdown, _pooled_match_count)
    raw_counts = {}
    summary_rows = []
    raw_arrays = {'catre_iou_thresholds': np.asarray(iou_thres_list)}
    for cls_id in range(1, num_classes):
        class_name = synset_names[cls_id]
        for index, iou_thres in enumerate(iou_thres_list):
            ...
            count = _compute_ap_breakdown(
                ...
                gt_matches)
            raw_counts[(cls_id, index)] = count
            summary_rows.append({
                'evaluator': 'CATRE', 'scope': 'category',
            ...
            prefix = 'catre_iou_{}_{}'.format(class_name, index)
    ...
    for index, iou_thres in enumerate(iou_thres_list):
        ...
        raw_counts[(num_classes, index)] = count
        summary_rows.append({
            'evaluator': 'CATRE', 'scope': 'overall_macro',
            'category': 'mean', 'metric': '3d_iou',
            'iou_threshold': iou_thres,
            'rotation_threshold_deg': np.nan,
            'translation_threshold_cm': np.nan,
            **count, 'ap_percent': iou_3d_aps[num_classes, index] * 100,
    ...
    if save_raw_details and report_path is not None:
        output_dir = os.path.dirname(report_path) or '.'
        counts_path = os.path.join(output_dir, 'evaluation_raw_counts.csv')
        matches_path = os.path.join(output_dir, 'evaluation_raw_matches.npz')
        old_summary = (pd.read_csv(counts_path)
                       if os.path.isfile(counts_path) else pd.DataFrame())
        pd.concat([old_summary, pd.DataFrame(summary_rows)], ignore_index=True).to_csv(
            counts_path, index=False)
        old_arrays = {}
        if os.path.isfile(matches_path):
            with np.load(matches_path) as saved_arrays:
                old_arrays = {key: saved_arrays[key] for key in saved_arrays.files}
        old_arrays.update(raw_arrays)
        np.savez_compressed(matches_path, **old_arrays)
    ...
    rows = [(cls_id, synset_names[cls_id])
            for cls_id in range(1, num_classes)]
    rows.append((num_classes, 'mean'))
    category_width = max(10, max(len(name) for _, name in rows))
    headers = ['IoU@{:.0f}'.format(thres * 100)
               for thres in iou_thres_list]
    value_width = max(10, max(len(header) for header in headers))
    header = '{:<{}} | {}'.format(
        'category', category_width,
        ' | '.join('{:>{}}'.format(item, value_width) for item in headers))
    lines.extend([header, '-' * len(header)])
    for row_id, class_name in rows:
        lines.append('{:<{}} | {}'.format(
            class_name, category_width,
            ' | '.join('{:>{}.1f}'.format(
                iou_3d_aps[row_id, index] * 100, value_width)
                for index in range(num_iou_thres))))
    report = '\n'.join(lines)

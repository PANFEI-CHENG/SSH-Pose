...
def compute_RT_errors(sRT_1, sRT_2, class_id, handle_visibility, synset_names):
    ...
    if synset_names[class_id] in ['bottle', 'can', 'bowl'] or \
        ...
        # if synset_names[class_id] in ['laptop', 'camera', 'mug']:
        #     # camera is symmetric when rotating around z-axis
        #     # 定义绕 y 轴旋转 180 度的矩阵
        #     R_y = np.array([[ -1,  0,  0],
        #                     [  0,  1,  0],
        #                     [  0,  0, -1]])
        #     R2 = R2 @ R_y
        #     R = R1 @ R2.transpose()
        #     cos_theta1 = (np.trace(R) - 1) / 2
        #     cos_theta = max(cos_theta, cos_theta1)
...
def compute_mAP(pred_results, out_dir, degree_thresholds=[180], shift_thresholds=[100],
    ...
    for progress, result in enumerate(tqdm(pred_results)):
        if select_class in ['laptop']:
            R_y = np.array([[ -1,  0,  0],
                    [  0,  1,  0],
                    [  0,  0, -1]])
            result['gt_RTs'][0][:3, :3] = result['gt_RTs'][0][:3, :3] @ R_y
        ...
        if len(gt_class_ids) == 0 and len(pred_class_ids) == 0:
            continue
        image = cv2.imread(result['img_path'])[:, :, :3]
        image = image[:, :, ::-1] #480*640*3
        intrinsics = result['intrinsics']
        parts = result['img_path'].split('/')
        part1 = parts[4]  # 0001
        part2 = parts[5]  # 1
        filename = os.path.splitext(parts[7])[0]  

        # 格式化输出
        framename = f"{part1}_{part2}_{int(filename):04d}"
        from utils.draw_utils import draw_detections        
        draw_detections(image, out_dir+'/draw', 'wild_test', framename, intrinsics,
                            result['pred_bboxes'], None, None, result['gt_RTs'], result['gt_scales'],

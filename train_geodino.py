...
def init():
    ...
    if cfg.mod == 'ts':
        cfg.log_dir = os.path.join('log3', args.dataset, 'PN2')
    elif cfg.mod == 'r':
        cfg.log_dir = os.path.join('log3', args.dataset, 'best_scaleppf')
    else:
        assert False, 'Wrong mode'

    if not os.path.isdir("log3"):
        os.makedirs("log3")
    if not os.path.isdir("log3/"+args.dataset):
        os.makedirs("log3/"+args.dataset)
    if not os.path.isdir(cfg.log_dir):
        os.makedirs(cfg.log_dir)

    # os.makedirs(cfg.log_dir, exist_ok=True)
    os.system(f'cp -r lib {cfg.log_dir}')
    os.system(f'cp -r model {cfg.log_dir}')
    os.system(f'cp -r provider {cfg.log_dir}')
    os.system(f'cp -r utils {cfg.log_dir}')
    os.system(f'cp  train_geodino.py {cfg.log_dir}')
    os.system(f'cp  test_geodino.py {cfg.log_dir}')
    os.system(f'cp  {args.config} {cfg.log_dir}')
    logger = get_logger(
        level_print=logging.INFO, level_save=logging.INFO, path_file=cfg.log_dir+"/training_logger.log")
